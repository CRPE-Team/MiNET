...
namespace MiNET
{
	public class Player : Entity, IMcpeMessageHandler
	{
		...
		public virtual void HandleMcpeMobEquipment(McpeMobEquipment message)
		{
			...
			else if (message.windowsId == 0x77)
			{
				if (message.slot != 1)
				{
				...
				SendEquipmentForPlayer(0x77, offHandItem);
			}
		}
		...
		{
			SendEquipmentForPlayer(0, Inventory.GetItemInHand(), receivers);
			SendEquipmentForPlayer(119, Inventory.OffHand, receivers);
		}

		protected virtual void SendEquipmentForPlayer(byte windowsId, Item item, Player[] receivers = null)
		{
			var mcpePlayerEquipment = McpeMobEquipment.CreateObject();
			mcpePlayerEquipment.runtimeEntityId = EntityId;
			mcpePlayerEquipment.windowsId = windowsId;
