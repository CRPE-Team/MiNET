...
using MiNET.Test.Utils.NiceLobby;
using MiNET.Utils;
...
namespace MiNET.Test.Utils.Code4Fun
{
	public class VideoCommand
	{
			...
			Task.Run(delegate
			{
				try
				{
					...
					Parallel.For(frameOffset, frameOffset + frameCount, (frame) =>
					{
						...
						var file = Path.Combine(@"C:\Development\Other\Smash Heroes 3x6 (128)\Smash Heroes 3x6 (128)", $"Smash Heroes Trailer{frame:D4}.bmp");
						if (!File.Exists(file))
						{
						...
						for (var x = 0; x < width; x++)
						{
							for (var y = 0; y < height; y++)
							{
								...
								var bitmapToBytes = BitmapToBytes(croppedImage, color);
					...
					var center = player.KnownPosition.GetCoordinates3D();
		...
		public void VideoX(Player player, int numberOfFrames, bool color)
		{
			Task.Run(delegate
			{
				try
				{
					...
					// 768x384
					for (var frame = frameOffset; frame < frameCount + frameOffset; frame++)
					{
						Log.Info($"Generating frame {frame}");
		...
		public static byte[] BitmapToBytes(Bitmap bitmap, bool useColor = false)
		{
			...
			for (var y = 0; y < bitmap.Height; y++)
			{
				for (var x = 0; x < bitmap.Width; x++)
				{
					...
					if (!useColor)
					{
						var rgb = (byte) ((color.R + color.G + color.B) / 3);
						bytes[i++] = rgb;
						bytes[i++] = rgb;
						bytes[i++] = rgb;
						bytes[i++] = 0xff;
		...
		{
			var batch = BatchUtils.CreateBatchPacket(new Memory<byte>(bytes, 0, (int) bytes.Length), CompressionLevel.Optimal, true);
			batch.MarkPermanent();
			batch.Encode();
