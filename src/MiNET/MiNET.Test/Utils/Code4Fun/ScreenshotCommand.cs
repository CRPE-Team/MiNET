...
namespace MiNET.Test.Utils.Code4Fun
{
	public class ScreenshotCommand
	{
		...
		private ConcurrentDictionary<Tuple<int, int>, PlayerMob> _mobs = new();
		...
		public void Screenshot(Player player)
		{
			...
			var pluginDirectory = Path.GetDirectoryName(new Uri(Assembly.GetExecutingAssembly().Location).LocalPath);

			var skinBytes = Skin.GetTextureFromFile(Path.Combine(pluginDirectory, "test_skin.png"));
			...
			for (var x = 0; x < _width; x++)
			{
				for (var y = 0; y < _height; y++)
				{
					...
					_mobs.TryAdd(new Tuple<int, int>(x, y), fake);
					fake.SpawnEntity();
					//fake.AddToPlayerList();
					//Thread.Sleep(500);
				}
			}
			_mobs.First().Value.Ticking += PlayerOnTicking;
		}
		...
		private void PlayerOnTicking(object sender, PlayerEventArgs playerEventArgs)
		{
			...
			foreach (var mobCoord in _mobs)
			{
				Log.Debug($"Updating {mobCoord.Key.Item1}, {mobCoord.Key.Item2}");
				PlayerMob mob = mobCoord.Value;
				mob.AddToPlayerList();

				var offsetx = (mobCoord.Key.Item1) * 62;
				var offsety = (_height - mobCoord.Key.Item2 - 1) * 62;
