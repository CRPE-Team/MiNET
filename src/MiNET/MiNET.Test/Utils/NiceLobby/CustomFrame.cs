...
namespace MiNET.Test.Utils.NiceLobby
{
	public class CustomItemFrame : ItemFrame
	{
		...
		public override bool PlaceBlock(Level world, Player player, BlockCoordinates blockCoordinates, BlockFace face, Vector3 faceCoords)
		{
			...
			var frame = new CustomFrame(_frames, itemFrameBlockEntity, world, _frameTicker)
			{
				Coordinates = coor,
			};
	...
	public class CustomFrame : Frame
	{
		...
		private void Tick(object state)
		{
			...
			try
			{
				...
				if (_itemFrameBlockEntity != null)
				{
					_itemFrameBlockEntity.SetItem(map, 0);
					_level.SetBlockEntity(_itemFrameBlockEntity);
				}
			}
			finally
			{
